...
struct EntryRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(entry.plateColor).font(Theme.captionFont).foregroundStyle(.secondary)
...
struct EntryFormView: View {
    ...
    @State private var plateColor: String
    @State private var dateInstalled: String
    @State private var notes: String
    @FocusState private var focusedField: Field?
    private enum Field { case f1, f2, f3, f4 }

    let existing: PlateEntry?
    let onSave: (PlateEntry) -> Void

    init(entry: PlateEntry?, onSave: @escaping (PlateEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
        _room = State(initialValue: entry?.room ?? "")
        _plateColor = State(initialValue: entry?.plateColor ?? "")
        _dateInstalled = State(initialValue: entry?.dateInstalled ?? "")
        ...
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    TextField("Platecolor", text: $plateColor)
                        .focused($focusedField, equals: .f2)
                        .accessibilityIdentifier("field_plateColor")
                    TextField("Dateinstalled", text: $dateInstalled)
            ...
            .navigationTitle(existing == nil ? "New Plate" : "Edit Plate")
            .toolbar {
